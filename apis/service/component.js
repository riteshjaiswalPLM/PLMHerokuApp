                    ...
                    if (attachmentDetails.attachmentSObject == "ContentVersion") {
                        ...
                        global.sfdc.sobject('ContentVersion').create(cvObj, function (cverr, ret) {
                            if (cverr || !ret.success) {
                                deleteContentVersion(cvIdArray);
                                return res.json({
                                    success: false,
...
var deleteContentVersion = function (attachmentIdArray) {
    attachmentIdArray.forEach(function (attachmentId) {
        global.sfdc.sobject('ContentVersion').destroy(attachmentId);
    });
};
componentRouter.post('/deleteexistingattachment', function (req, res) {
    ...
    global.sfdc.sobject('ContentDocument')
        .find({ id: attachment.ContentDocument.Id })
        .destroy(function (err, rets) {
            if (err) {
                ...
                if (rets.length > 0) {
                    return res.json({
                        success: true,
                        filename: attachment.ContentDocument.Title,
                        message: "File Deleted successfully",
                        BodyLength: attachment.ContentDocument.ContentSize
                    });
                }
                else {
                    return res.json({
    ...
    global.sfdc
        ...
        .execute(function (err, records) {
            if (err) {
                return res.json({
                    success: false,
                    message: 'Error occured while loading line item.',
                    error: err
                });
            }
            else if (records.length === 0) {
