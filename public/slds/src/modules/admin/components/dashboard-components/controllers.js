...
admin.controller('AdminDashboardComponentsEditController', [
    '$scope', '$state', '$stateParams', '$dialog', '$adminLookups', 'sobjectService', 'blockUI', 'genericComponentService', 'mobileLayoutService', '$adminModals',
    function ($scope, $state, $stateParams, $dialog, $adminLookups, sobjectService, blockUI, genericComponentService, mobileLayoutService, $adminModals) {
                ...
                genericComponentService.loadComponentDetails($scope.component)
                    .success(function (response) {
                        ...
                        if (response.success === true) {
                            ...
                            if ($scope.component.detailSObject != undefined || $scope.component.detailSObject != null) {
                                $scope.multipleField = angular.copy($scope.component.detailSObject.SObjectFields);
                                $scope.referenceSObjectNames = [];
                                $scope.multipleField.forEach(function (field) {
                                    if (field.type === 'reference') {
                                        field.referenceTo.forEach(function (reference) {
                                ...
                                $scope.multipleField = angular.copy($scope.component.SObject.SObjectFields);
                                $scope.referenceSObjectNames = [];
                                $scope.multipleField.forEach(function (field) {
                                    if (field.type === 'reference') {
                                        field.referenceTo.forEach(function (reference) {
            ...
            $adminLookups.sObject(whereClause, function (sObject) {
                ...
                if (SObject !== 'detailSObject') {
                    if (sObject === undefined) {
                        ...
                        $scope.component.ComponentDetails[0].configuration.multipleFields = [];
                        ...
                        if ($scope.component.detailSObject == null || $scope.component.detailSObject == undefined) {
                            $scope.multipleField = angular.copy($scope.component.SObject.SObjectFields);
                            $scope.referenceSObjectNames = [];
                            $scope.multipleField.forEach(function (field) {
                                if (field.type === 'reference') {
                    ...
                    if (sObject === undefined) {
                        ...
                        $scope.component.ComponentDetails[0].configuration.multipleFields = [];
                        $scope.multipleField = []
                        if ($scope.component.SObject != undefined || $scope.component.SObject != null) {
                            $scope.multipleField = angular.copy($scope.component.SObject.SObjectFields);
                            $scope.referenceSObjectNames = [];
                            $scope.multipleField.forEach(function (field) {
                                if (field.type === 'reference') {
                        ...
                        $scope.component.detailSObject = sObject;
                        $scope.component.ComponentDetails[0].configuration.multipleFields = [];
                        $scope.multipleField = angular.copy(sObject.SObjectFields);
                        $scope.referenceSObjectNames = [];
                        $scope.multipleField.forEach(function (field) {
                            if (field.type === 'reference') {
                                field.referenceTo.forEach(function (reference) {
        ...
        $scope.addToMultipleFields = function (multipleField) {
            var duplicate = [];
            if ($scope.component.ComponentDetails[0].configuration.multipleFields == undefined) {
                $scope.component.ComponentDetails[0].configuration.multipleFields = [];
            }
            $scope.component.ComponentDetails[0].configuration.multipleFields.forEach(function (_multipleField) {
                if (_multipleField.SObjectField.name === multipleField.name) {
                    duplicate.push(_multipleField);
                }
            });
            if (duplicate.length === 0 || multipleField.type === 'reference') {
                var ControllerSObjectField = undefined;
                if (multipleField.controllerName && (multipleField.type === 'picklist' || multipleField.type === 'multipicklist')) {
                    angular.forEach($scope.multipleField, function (ctrlField) {
                        if (ctrlField.name === multipleField.controllerName) {
                            ControllerSObjectField = ctrlField;
                        }
                    });
                }
                $scope.component.ComponentDetails[0].configuration.multipleFields.push({
                    SObjectField: multipleField,
        ...
        $scope.onChangeRequired = function (multipleField) {
            if (multipleField.required) {
                multipleField.readonly = false;
                multipleField.hidden = false;
            }
        }
        $scope.onChangeReadonly = function (multipleField) {
            if (multipleField.readonly) {
                multipleField.required = false;
                multipleField.hidden = false;
            }
        }
        $scope.onChangeMultipleHidden = function (multipleField) {
            if (multipleField.hidden) {
                multipleField.required = false;
                multipleField.readonly = false;
            }
        }
        $scope.openFieldCriteriaModal = function (multipleField) {
            $adminModals.criteriaModal({
                title: 'Field Criteria | ' + multipleField.label,
                fields: $scope.multipleField,
                criteria: multipleField.criteria ? multipleField.criteria : null
            }, function (criteria) {
                multipleField.criteria = criteria;
            });
        };
